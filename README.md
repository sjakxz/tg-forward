...
| `alert_source_chat_ids` | int 数组 | 触发告警的来源白名单，是 `source_chat_ids` 的子集。仅当消息来自这里列出的来源时，才向 `alert_chat_ids` 发送「1」催办；转发本身不受此配置影响。**留空/不填即所有 `source_chat_ids` 都触发告警（默认行为）**。 |
